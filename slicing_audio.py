...
def slicing_audio(audio_track, name, number, seconds, overlap, class_no, cat):
    ...
    for milsec in range(0, len(audio_track), milliseconds):
        ...
        if slice_end < len(audio_track):
            ...
            slice.export(
                out_f=f"audio_files/class_{class_no}/{cat}/seconds_{seconds}_overlap_{overlap}/{name}_{number}_slice_{int(milsec/1000)}.mp3",
                format="mp3")

def perform_on_class(name, number, seconds, overlap, class_no, cat):
...
# arguments: name, number, seconds, overlap, class, category
perform_on_class("ASMR_train", 14, 1, 0, 1, "Train")